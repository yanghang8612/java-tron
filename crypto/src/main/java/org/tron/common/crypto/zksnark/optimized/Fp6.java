...
class Fp6 implements Field<Fp6> {
  ...
  /** Multiplication by c0 + c1*v (v^3 = 9+i), using five Fp2 multiplications. */
  Fp6 mulBy01(Fp2 c0, Fp2 c1) {
    Fp2 v0 = a.mul(c0);
    Fp2 v1 = b.mul(c1);
    Fp2 ra = v0.add(c.mul(c1).mulByNonResidue());
    Fp2 rb = a.add(b).mul(c0.add(c1)).sub(v0).sub(v1);
    Fp2 rc = c.mul(c0).add(v1);
    return new Fp6(ra, rb, rc);
  }
