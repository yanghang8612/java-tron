...
import static org.tron.common.crypto.zksnark.optimized.Params.TWIST_MUL_BY_P_X;
import static org.tron.common.crypto.zksnark.optimized.Params.TWIST_MUL_BY_P_Y;
...
public class BN128G2 extends BN128Fp2 {
  ...
  public static BN128G2 create(byte[] a, byte[] b, byte[] c, byte[] d) {
    return create(a, b, c, d, Long.MAX_VALUE);
  }

  /** Returns null for an invalid point or interrupted subgroup validation. */
  public static BN128G2 create(byte[] a, byte[] b, byte[] c, byte[] d,
      long deadlineInUs) {
    ...
    if (!G2SubgroupCheck.isGroupMember(p, deadlineInUs)) {
      return null;
    }
