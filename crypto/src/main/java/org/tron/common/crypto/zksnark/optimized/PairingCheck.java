...
import static org.tron.common.crypto.zksnark.optimized.Params.B_Fp2;
import static org.tron.common.crypto.zksnark.optimized.Params.PAIRING_FINAL_EXPONENT_Z;
...
public class PairingCheck {
  ...
  private static final byte[] LOOP_NAF = FixedExponent.naf(LOOP_COUNT);

  List<Pair> pairs = new ArrayList<>();
  Fp12 product = Fp12._1;

  private final long deadlineInUs;

  private PairingCheck(long deadlineInUs) {
    this.deadlineInUs = deadlineInUs;
  }
  ...
  public static PairingCheck create(long deadlineInUs) {
    return new PairingCheck(deadlineInUs);
  }
  ...
  private Fp12 millerLoop() {
    ...
    BN128G2[] negatives = new BN128G2[active.size()];
    for (int k = 0; k < active.size(); k++) {
      ...
      BN128G2 q = addends[k];
      negatives[k] = new BN128G2(q.x, q.y.negate(), q.z);
    }
    ...
    Fp12 f = Fp12._1;
    // Signed NAF, as in gnark-crypto's BN254 MillerLoop. Negative digits use -Q;
    // the omitted vertical-line factors lie in Fp6 and vanish in final exponentiation.
    for (int i = LOOP_NAF.length - 2; i >= 0; i--) {
      if (ExecutionDeadline.isExceeded(deadlineInUs)) {
        return null;
      }
      if (i != LOOP_NAF.length - 2) { // the first square would be 1^2
        f = f.squared();
      }
      for (int k = 0; k < active.size(); k++) {
        if (ExecutionDeadline.isExceeded(deadlineInUs)) {
        ...
        if (LOOP_NAF[i] != 0) {
          BN128G2 base = LOOP_NAF[i] > 0 ? pair.g2 : negatives[k];
          Precomputed addition = flippedMillerLoopMixedAddition(base, addends[k]);
          addends[k] = addition.g2;
          f = multiplyLinePair(f, pair.g1, doubling.coeffs, addition.coeffs);
        } else {
    ...
    for (int k = 0; k < active.size(); k++) {
      ...
      Precomputed last = flippedMillerLoopMixedAddition(q2, addition.g2);
      f = multiplyLinePair(f, pair.g1, addition.coeffs, last.coeffs);
    }
    return f;
  }
  ...
  private static Fp12 multiplyLinePair(Fp12 f, BN128G1 g1, EllCoeffs c, EllCoeffs d) {
    Fp12 lines = Fp12.multiplySparse024(c.ell0, g1.x.mul(c.ellVV), g1.y.mul(c.ellVW),
        d.ell0, g1.x.mul(d.ellVV), g1.y.mul(d.ellVW));
    return f.mulBy01234(lines);
  }
  ...
  public static Fp12 finalExponentiation(Fp12 el) {
    return finalExponentiation(el, Long.MAX_VALUE);
  }

  /** Returns null if exponentiation is interrupted by the execution deadline. */
  static Fp12 finalExponentiation(Fp12 el, long deadlineInUs) {
    ...
    // last chunk
    Fp12 a = pre.negExp(PAIRING_FINAL_EXPONENT_Z, deadlineInUs);
    if (a == null) {
      return null;
    }
    ...
    Fp12 d = c.mul(b);
    Fp12 e = d.negExp(PAIRING_FINAL_EXPONENT_Z, deadlineInUs);
    if (e == null) {
      return null;
    }
    Fp12 f = e.cyclotomicSquared();
    Fp12 g = f.negExp(PAIRING_FINAL_EXPONENT_Z, deadlineInUs);
    if (g == null) {
      return null;
    }
  ...
  /**
   * Returns whether execution completed, not whether the pairing equation holds (see result()).
   * Interruption invalidates the accumulator so a partial product cannot be reported as success.
   */
  public boolean run() {
    if (product == null) {
      return false;
    ...
    if (!product.equals(Fp12._1)) {
      product = finalExponentiation(product, deadlineInUs);
    }
    return product != null;
  }

  public int result() {
    return Fp12._1.equals(product) ? 1 : 0;
  }
