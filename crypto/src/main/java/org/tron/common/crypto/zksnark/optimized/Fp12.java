...
class Fp12 implements Field<Fp12> {
  ...
  /**
   * Product of two lines (a0,0,a2,0,a4,0) and (b0,0,b2,0,b4,0).
   * Line fusion follows gnark's approach, re-derived for OUR 024 (not its 034) layout.
   * With w^2=v, v^3=xi, the result has coefficient 5 zero. Six Fp2 products suffice.
   */
  static Fp12 multiplySparse024(Fp2 a0, Fp2 a2, Fp2 a4, Fp2 b0, Fp2 b2, Fp2 b4) {
    Fp2 p0 = a0.mul(b0);
    Fp2 p2 = a2.mul(b2);
    Fp2 p4 = a4.mul(b4);
    Fp2 p02 = a0.add(a2).mul(b0.add(b2)).sub(p0).sub(p2);
    Fp2 p04 = a0.add(a4).mul(b0.add(b4)).sub(p0).sub(p4);
    Fp2 p24 = a2.add(a4).mul(b2.add(b4)).sub(p2).sub(p4);
    return new Fp12(new Fp6(p0.add(p4.mulByNonResidue()), p2.mulByNonResidue(), p02),
        new Fp6(p24.mulByNonResidue(), p04, Fp2.ZERO));
  }

  /** Multiplication by a fused line product; o.b.c MUST be zero. */
  Fp12 mulBy01234(Fp12 o) {
    Fp6 aa = a.mul(o.a);
    Fp6 bb = b.mulBy01(o.b.a, o.b.b);
    Fp6 ra = aa.add(bb.mulByNonResidue());
    Fp6 rb = a.add(b).mul(o.a.add(o.b)).sub(aa).sub(bb);
    return new Fp12(ra, rb);
  }
  ...
  Fp12 cyclotomicExp(BigInteger pow) {
    return cyclotomicExp(pow, Long.MAX_VALUE);
  }

  private Fp12 cyclotomicExp(BigInteger pow, long deadlineInUs) {
  ...
  Fp12 negExp(BigInteger exp) {
    return negExp(exp, Long.MAX_VALUE);
  }

  /** Returns null if the execution deadline is exceeded in either exponentiation loop. */
  Fp12 negExp(BigInteger exp, long deadlineInUs) {
    Fp12 result;
    if (exp.equals(Params.PAIRING_FINAL_EXPONENT_Z)) {
      result = cyclotomicExpBySeed(deadlineInUs);
    } else {
      result = cyclotomicExp(exp, deadlineInUs);
    }
    return result == null ? null : result.unitaryInverse();
  }
  ...
  private Fp12 cyclotomicExpBySeed(long deadlineInUs) {
