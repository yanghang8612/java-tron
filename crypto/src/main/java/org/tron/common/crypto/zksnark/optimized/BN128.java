...
public abstract class BN128<T extends Field<T>> {
  ...
  public BN128<T> mul(BigInteger s) {
    return mul(s, Long.MAX_VALUE);
  }

  /** Returns null if the execution deadline is exceeded during scalar multiplication. */
  public BN128<T> mul(BigInteger s, long deadlineInUs) {
  ...
  // Package-private for the exact fixed-seed addition chain used by G2 validation.
  BN128<T> dbl() {
