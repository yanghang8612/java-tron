...
public class EventConfigTest {
  ...
  public void testDefaults() {
    ...
    assertTrue(ec.getJustlendTokens().isEmpty());
    assertEquals("", ec.getJustlendRentMarket());
  }
  ...
  @Test
  public void testJustlendConfig() {
    Config config = withRef(
        "event.subscribe.justlend {"
            + " token.list = [\"TToken1\", \"TToken2\"],"
            + " rent.market = \"TRentMarket\" }");
    EventConfig ec = EventConfig.fromConfig(config);
    assertEquals(2, ec.getJustlendTokens().size());
    assertEquals("TToken1", ec.getJustlendTokens().get(0));
    assertEquals("TRentMarket", ec.getJustlendRentMarket());
  }
