...
public class EventConfig {
  ...
  // === JustLend Feature ===
  // Legacy nested keys (justlend.token.list / justlend.rent.market) are read manually in
  // fromConfig; @Setter(NONE) keeps ConfigBeanFactory from requiring a matching property key.
  @Setter(lombok.AccessLevel.NONE)
  private List<String> justlendTokens = new ArrayList<>();
  @Setter(lombok.AccessLevel.NONE)
  ...
  public static EventConfig fromConfig(Config config) {
    ...
    // === JustLend Feature ===
    // legacy nested keys: event.subscribe.justlend.{token.list, rent.market}
    if (section.hasPath("justlend.token.list")) {
      ec.justlendTokens = section.getStringList("justlend.token.list");
    }
    if (section.hasPath("justlend.rent.market")) {
      ec.justlendRentMarket = section.getString("justlend.rent.market");
    }
