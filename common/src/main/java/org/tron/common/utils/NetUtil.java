...
public class NetUtil {
  ...
  public static double getPrice(String symbol) {
    for (int i = 0; i < 3; i++) {
      try {
        String repStr =
            get("https://c.tronlink.org/v1/cryptocurrency/getprice?convert=USD&symbol=" + symbol);
        JsonElement element = JsonParser.parseString(repStr);
        return element.getAsJsonObject()
            .get("data").getAsJsonObject()
            .get(symbol).getAsJsonObject()
