...
public class FreezeTrackerCapsule extends TriggerCapsule {
  ...
  @Getter @Setter private FreezeBalanceTrigger freezeBalanceTrigger;

  public FreezeTrackerCapsule(
      BlockCapsule block,
      Map<String, FreezeChangeRecord.FreezeInfo> freezeInfoMap,
    ...
    freezeInfoMap.forEach(
        (key, item) -> {
          if (item.getIncrementFreezeEnergyBalance() != 0) {
            FreezeBalanceTrigger.FreezeBalance freezeBalance =
                new FreezeBalanceTrigger.FreezeBalance();
            ...
            freezeBalance.setIncrementFreezeBalance(
                String.valueOf(item.getIncrementFreezeEnergyBalance()));
            freezeBalance.setIncrementExpireTime(
                String.valueOf(item.getIncrementExpireTimeForEnergy()));
          ...
          if (item.getIncrementFreezeBandwidthBalance() != 0) {
            ...
            freezeBalance.setIncrementFreezeBalance(
                String.valueOf(item.getIncrementFreezeBandwidthBalance()));
            freezeBalance.setIncrementExpireTime(
                String.valueOf(item.getIncrementExpireTimeForBandwidth()));
  ...
  private void handlerTrc10Transfer(
      TransactionCapsule item, List<FreezeBalanceTrigger.AssetTransfer> transferList) {
    try {
      AssetIssueContractOuterClass.TransferAssetContract transferContract =
          item.getInstance()
              .getRawData()
              .getContract(0)
              .getParameter()
