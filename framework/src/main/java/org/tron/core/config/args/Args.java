...
public class Args extends CommonParameter {
  ...
  private static void applyEventConfig(EventConfig ec) {
    ...
    // === JustLend Feature ===
    if (!CollectionUtils.isEmpty(ec.getJustlendTokens())) {
      epc.setJustlendTokens(ec.getJustlendTokens());
    }

    // === JustLend Feature ===
    if (StringUtils.isNotEmpty(ec.getJustlendRentMarket())) {
      epc.setJustlendRentMarket(ec.getJustlendRentMarket());
    }
