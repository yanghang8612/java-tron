...
public class P2pEventHandlerImpl extends P2pEventHandler {
  ...
  private void processMessage(PeerConnection peer, byte[] data) {
    ...
    try {
      ...
      if (FAST_SYNC_STATS_MODE && isFastIgnoredMessage(msg, type)) {
        return;
      }
      ...
      long slowMessageThreshold = FAST_SYNC_STATS_MODE ? FAST_SYNC_SLOW_MESSAGE_MS : 50L;
      if (costs > slowMessageThreshold) {
        ...
        if (!FAST_SYNC_STATS_MODE && type != null) {
