...
public class ArgsTest {
  ...
  @Test
  public void testEventConfigWiresJustlendSettings() {
    Config config = ConfigFactory.parseString(
            "storage.db.directory=database, event.subscribe { enable=true, justlend {"
                + " token.list=[\"TToken1\",\"TToken2\"], rent.market=\"TRentMarket\" } }")
        .withFallback(ConfigFactory.defaultReference());
    Args.applyConfigParams(config);
    Args.applyEventConfig();
    Assert.assertEquals(2,
        Args.getInstance().getEventPluginConfig().getJustlendTokens().size());
    Assert.assertEquals("TRentMarket",
        Args.getInstance().getEventPluginConfig().getJustlendRentMarket());
    Args.clearParam();
  }
