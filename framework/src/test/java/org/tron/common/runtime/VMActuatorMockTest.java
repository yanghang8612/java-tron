...
public class VMActuatorMockTest {
  ...
  @Test
  public void constantCallClearsJumpTableOnEarlyReturn() throws Exception {
    try (MockedStatic<VM> vmMock = Mockito.mockStatic(VM.class)) {
      ...
      // The constant call must receive a private table, not the shared consensus one.
      JumpTable shared = OperationRegistry.getTable(false);
      vmMock.verify(() -> VM.play(any(), argThat(table -> table != shared)));

      try {
        OperationRegistry.getTable(true);
        Assert.fail("constant-call JumpTable must be cleared after execution");
  ...
  private void runCatchPathTest(Throwable thrownByVm, boolean osakaOn, int expectedSize)
      throws Exception {
    runCatchPathTest(thrownByVm, osakaOn, expectedSize, false);
  }

  private void runCatchPathTest(Throwable thrownByVm, boolean osakaOn, int expectedSize,
      boolean constantCall) throws Exception {
    ...
    try (MockedStatic<VM> vmMock = Mockito.mockStatic(VM.class)) {
      ...
      VMActuator actuator = new VMActuator(constantCall);
      Field f = VMActuator.class.getDeclaredField("program");
      f.setAccessible(true);
      f.set(actuator, program);
  ...
  @Test
  public void constantCallClearsJumpTableOnFailure() throws Exception {
    try {
      runCatchPathTest(new RuntimeException("boom"), false, 1, true);

      try {
        OperationRegistry.getTable(true);
        Assert.fail("constant-call JumpTable must be cleared after failure");
      } catch (IllegalStateException expected) {
        // expected
      }
    } finally {
